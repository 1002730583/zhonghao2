...
function change_images(uploadid,returnid){
	...
	var in_name = d.$("#att-name").html().substring(1);
	//var str = $('#'+returnid).html();
	var names = in_name.split('|');
	...
	$.each( contents, function(i, n) {
		if(n){
			...
			str += "<div id='image"+ids+"'><input type='text' name='"+returnid+"_url[]' value='"+n+"' style='width:360px;' ondblclick='image_priview(this.value);' class='input-text'> <input type='text' name='"+returnid+"_alt[]' value='"+names[i]+"' style='width:100px;' class='input-text' onfocus=\"if(this.value == this.defaultValue) this.value = ''\" onblur=\"if(this.value.replace(' ','') == '') this.value = this.defaultValue;\"> <a href=\"javascript:remove_div('image"+ids+"')\">移除</a> </div><div class='bk10'></div>";
		}
		});
	$('#'+returnid).html(str);
}
